@php
    $couponCount =
        $couponCount ?? (Auth::check() ? Auth::user()->customerVouchers()->where('is_redeemed', false)->count() : 0);
    $pointsBalance = $pointsBalance ?? (Auth::check() ? Auth::user()->points_balance : 0);
@endphp

<a href="{{ route('customer.points') }}" class="text-decoration-none text-dark">
    <div class="coupon-info-bar mb-3">
        <div class="d-flex justify-content-between align-items-center p-3 bg-white rounded shadow-sm clickable-card">
