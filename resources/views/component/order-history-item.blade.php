    ...
    <div class="d-flex align-items-start justify-content-between">
        <div class="order-info flex-grow-1">
            <div class="d-flex align-items-center mb-1">
                ...
                <div>
                    <h6 class="order-id mb-0">Order #{{ $order->id }}</h6>
                </div>
            </div>
            <div class="order-details-text">
                <p class="mb-0 service-type">{{ $order->laundryType->name ?? 'Regular Laundry' }} {{ $order->weight_kg }} kg</p>
                <p class="mb-0 order-status">{{ ucfirst($order->laundry_status) }}</p>
                <p class="mb-0 total-price fw-bold">Total {{ $order->weight_kg }} kg, Rp {{ number_format($order->total_price, 0, ',', '.') }}</p>
