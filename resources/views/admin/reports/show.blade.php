...
@section('title', __('admin_transaction_detail.title'))
@section('page_title', __('admin_transaction_detail.page_title'))

@section('content')

<h2 class="mb-4">{{ __('admin_transaction_detail.page_title') }}</h2>

<div class="card shadow-sm p-4">

    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="m-0">
            {{ __('admin_transaction_detail.order_code') }} <span style="color:red">#{{ $transaction->order_code }}</span>
        </h3>
    ...
    <div class="card shadow-sm rounded-3 p-3 mb-3">
        <div class="card-body">
            ...
            <!-- Laundry Type -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.laundry_type') }}</h6>
                <span class="fw-semibold">{{ $transaction->laundryType->name }}</span>
            </div>

            <!-- Laundry Weight -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.laundry_weight') }}</h6>
                <span class="fw-semibold">{{ $transaction->weight_kg }} kg</span>
            </div>

            <!-- Laundry Price / Kg -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.price_per_kg') }}</h6>
            ...
            <!-- Subtotal -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.subtotal') }}</h6>
                <span class="fw-semibold">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
            </div>

            <!-- Voucher -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.voucher') }}</h6>
                <span class="fw-semibold">Rp {{ number_format($discount, 0, ',', '.')}}</span>
            </div>

            <!-- Total Price -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.total_price') }}</h6>
    ...
    <div class="card shadow-sm rounded-3 p-3">
        <div class="card-body">
            <h3 class="mb-3 pb-2 border-bottom">
                {{ __('admin_transaction_detail.customer_details') }}
            </h3>

            <!-- Customer Name -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.customer_name') }}</h6>
                <span class="fw-semibold">{{ $transaction->account->name }}</span>
            </div>

            <!-- Email -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.email') }}</h6>
                <span class="fw-semibold">{{ $transaction->account->email }}</span>
            </div>

            <!-- Phone Number -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.phone') }}</h6>
                <span class="fw-semibold">{{ $transaction->account->telephone }}</span>
            </div>

            <!-- Gender -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.gender') }}</h6>
                <span class="fw-semibold">{{ $transaction->account->gender }}</span>
            </div>

            <!-- Address -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.address') }}</h6>
                <span class="fw-semibold">{{ $transaction->account->address }}</span>
            </div>

            <!-- Notes -->
            <div class="d-flex justify-content-between py-2 mb-2 border-bottom">
                <h6>{{ __('admin_transaction_detail.notes') }}</h6>
